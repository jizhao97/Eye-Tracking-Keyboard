...
class GUI(threading.Thread):
    def button_click(self, buttonNum, eye = 0, trigger=False):#TODO change eye=0
        ...
        if self.windowPosition == 1:  # char window
            ...
            if buttonNum == 0:
                ...
                self.middleButton_click()
            elif buttonNum ==5and self.parser.getint('settings', 'eyeMode')==1:
                self.windowPosition=1
                self.btn_str[0].set(".")
                self.btn_str[1].set(",")
                self.btn_str[2].set("?")
    ...
    def cameraCalibrationUpdate(self, message):
        msg = "Calabrating you camera! \n\nPlease look "
        if message == "U":
            msg = msg + "up."
        elif message == "D":
            msg = msg + "down."
    ...
    def run(self, calabrationMSG = ""):
        ...
        if calabrationMSG != "":
            self.btn_str[4].set("Calabrating you camera! \n\nPlease look up.")
        else:
            if self.parser.getint('settings', 'eyeMode') == 1:  # one word per button
